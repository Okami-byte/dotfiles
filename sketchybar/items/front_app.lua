...
local function update_windows(windows)
	...
	-- Filter windows to exclude specific apps without titles
	...
	-- Determine truncation based on number of windows
	local window_count = #filtered_windows
	...
	for _, window in ipairs(filtered_windows) do
		-- Truncate label based on window count
		local window_label
		if max_length then
